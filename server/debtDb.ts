...
export async function getDebtReport(params: {
  ...
  const targetRowsRaw = await db.execute(sql`
    SELECT LEFT(${installments.dueDate}::text, 7) AS month,
  ...
  const collectedRowsRaw = await db.execute(sql`
    SELECT LEFT(${paymentTransactions.paidAt}::text, 7) AS month,
...
export type PayRawRow = {
  ...
  /** วันที่สร้าง record (created_at จาก payment_transactions) */
...

  {
    ...
    const rawCloseAmtData = await db.execute(sql`
      SELECT contract_external_id,
             ...
             CAST((raw_json::jsonb->>'close_installment_amount') AS DECIMAL(18,2)) AS close_installment_amount,
             DATE(paid_at) AS paid_date,
             receipt_no
        FROM ${paymentTransactions}
       WHERE ${paymentTransactions.section} = ${params.section}
         AND (raw_json::jsonb->>'close_installment_amount') IS NOT NULL
         AND CAST((raw_json::jsonb->>'close_installment_amount') AS DECIMAL(18,2)) > 0
    `);
...

  {
    ...
    const rawCloseData = await db.execute(sql`
      SELECT contract_external_id,
             ...
             receipt_no,
             CAST((raw_json::jsonb->>'overpaid_amount') AS DECIMAL(18,2)) AS overpaid_amount,
             CAST((raw_json::jsonb->>'principal_paid') AS DECIMAL(18,2)) AS principal_paid,
             CAST((raw_json::jsonb->>'interest_paid') AS DECIMAL(18,2)) AS interest_paid,
             CAST((raw_json::jsonb->>'fee_paid') AS DECIMAL(18,2)) AS fee_paid,
             CAST((raw_json::jsonb->>'close_installment_amount') AS DECIMAL(18,2)) AS close_installment_amount,
             CAST((raw_json::jsonb->>'bad_debt_amount') AS DECIMAL(18,2)) AS bad_debt_amount,
             CAST((raw_json::jsonb->>'payment_id') AS BIGINT) AS payment_id,
             CAST(amount AS DECIMAL(18,2)) AS total_paid_amount,
             paid_at
        FROM ${paymentTransactions}
       WHERE ${paymentTransactions.section} = ${params.section}
    ...
    for (const pr of allPayRows) {
      ...
      const payRow: PayRawRow = {
        ...
        created_at: (pr as any).created_at ? String((pr as any).created_at) : null,
      };
...
export async function listDebtCollected(params: { section: SectionKey }) {
  ...
  // updated_by/updated_at are now stored directly in payment_transactions columns
  // (populated from API response during sync — no need to JOIN installments anymore)
  const payRowsRaw = await db.execute(sql`
    SELECT pt.contract_external_id,
           ...
           CAST((pt.raw_json::jsonb->>'principal_paid')           AS DECIMAL(18,2)) AS principal_paid,
           CAST((pt.raw_json::jsonb->>'interest_paid')            AS DECIMAL(18,2)) AS interest_paid,
           CAST((pt.raw_json::jsonb->>'fee_paid')                 AS DECIMAL(18,2)) AS fee_paid,
           CAST((pt.raw_json::jsonb->>'penalty_paid')             AS DECIMAL(18,2)) AS penalty_paid,
           CAST((pt.raw_json::jsonb->>'unlock_fee_paid')          AS DECIMAL(18,2)) AS unlock_fee_paid,
           CAST((pt.raw_json::jsonb->>'discount_amount')          AS DECIMAL(18,2)) AS discount_amount,
           CAST((pt.raw_json::jsonb->>'overpaid_amount')          AS DECIMAL(18,2)) AS overpaid_amount,
           CAST((pt.raw_json::jsonb->>'close_installment_amount') AS DECIMAL(18,2)) AS close_installment_amount,
           CAST((pt.raw_json::jsonb->>'bad_debt_amount')          AS DECIMAL(18,2)) AS bad_debt_amount,
           CAST((pt.raw_json::jsonb->>'payment_id')               AS BIGINT) AS payment_id,
           NULL AS installment_external_id,
           pt.receipt_no AS receipt_no,
           (pt.raw_json::jsonb->>'remark')     AS remark,
     ...
     ORDER BY pt.contract_external_id, pt.paid_at, CAST((pt.raw_json::jsonb->>'payment_id') AS BIGINT)
  `);
  ...
  const rows = baseRows.map((c) => {
    ...
    // sortedRealForBadDebt ใช้ร่วมกันระหว่าง if block และ normalPaymentsRaw filter
    let sortedRealForBadDebt: typeof realPaymentsRaw = [];
    if (c.debtStatus === "หนี้เสีย" && realPaymentsRaw.length > 0) {
      ...
      if (primaryTotal >= 1000) {
        ...
        const fallbackTotal = fallbackBatch.reduce(
          (sum, p) => sum + Number((p as any).total_paid_amount ?? 0), 0
        );
      ...
      // bad_debt_date = paid_at ของ row ล่าสุดใน batch (ระดับ 1 ใช้ latestPaidDate, ระดับ 2 ใช้ paid_at ล่าสุดใน fallback batch)
      ...
      // sortedRealForBadDebt ใช้สำหรับ filter normalPayments ด้านล่าง
      sortedRealForBadDebt = sortedReal;
      // เก็บ finalBatch สำหรับ filter normalPayments
      (sortedRealForBadDebt as any).__finalBatch = finalBatch;
      (sortedRealForBadDebt as any).__isFallback = isFallback;
      (sortedRealForBadDebt as any).__latestCreatedDate = latestCreatedDate;
      (sortedRealForBadDebt as any).__latestUpdatedBy = latestUpdatedBy;
    }
    ...
    if (contractBadDebtAmount != null && contractBadDebtAmount > 0 && contractBadDebtDate) {
      ...
      // Phase 106: normalPayments = all real payments EXCEPT those in the bad-debt batch
      // Phase 63 fix: ใช้ installmentAmount เป็น fallback เมื่อ amount=0
      const baselineAmt = c.installmentAmount ?? 0;
      // ดึง finalBatch จาก sortedRealForBadDebt ที่เก็บไว้
      const _finalBatch: Set<unknown> = new Set(
        ((sortedRealForBadDebt as any).__finalBatch ?? []) as unknown[]
      );
      const _isFallback: boolean = (sortedRealForBadDebt as any).__isFallback ?? false;
      const _bdCreatedDate: string | null = (sortedRealForBadDebt as any).__latestCreatedDate ?? null;
      const _bdUpdatedBy: string | null = (sortedRealForBadDebt as any).__latestUpdatedBy ?? null;
      const normalPaymentsRaw = realPaymentsRaw.filter((p) => {
        if (_finalBatch.has(p)) return false; // อยู่ใน bad-debt batch → filter ออก
        if (_isFallback) {
          // Fallback: ถ้า created_date + updated_by ตรงกัน = อยู่ใน batch (filter ออก)
          const pCreatedDate = ((p as any).created_at ?? "").substring(0, 10);
          const pUpdatedBy   = (p as any).updated_by ?? null;
...
export async function listWatchGroup(params: {
  ...
  for (const s of contractRows) {
    ...
    if (inst1Done && due2Reached && inst2NotDone) {
      arrearsCount = 1;
    } else if (due1Reached && inst1NotDone) {
      arrearsCount = 0;
    }
