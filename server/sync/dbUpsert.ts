...
/** Pull a short Postgres cause from Drizzle's nested "Failed query" errors. */
function contractsInsertErrorMessage(err: unknown): string {
  const raw =
    (err as { cause?: { message?: string } })?.cause?.message ??
    (err as { message?: string })?.message ??
    String(err);
  const colMissing = raw.match(/column "([^"]+)" of relation "contracts" does not exist/i);
  if (colMissing) return `contracts: column "${colMissing[1]}" does not exist — run schema migration on fastfone-db`;
...
export async function upsertContracts(rows: AnyRow[], section: SectionKey): Promise<number> {
  ...
  for (const batch of chunks(normalizeRows(rows), BATCH_SIZE)) {
    ...
    try {
      ...
      const msg = contractsInsertErrorMessage(insertErr);
      console.error(`[upsertContracts] ${section} batch failed (${merged.length} rows):`, msg);
      throw new Error(msg);
    }
    total += merged.length;
  }
