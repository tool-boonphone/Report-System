  ...
  // (ไม่แยก per due_month เพราะ live query ก็ไม่แยก)
  {
    ...
    const q = `
      SELECT
        ...
        base.product_type,
        CASE
          WHEN base.model ILIKE 'iPhone%' OR base.model ILIKE 'iPad%' THEN 'iOS'
          WHEN base.model IS NOT NULL AND base.model != '' THEN 'Android'
          ELSE NULL
        END AS device_family,
        COUNT(DISTINCT base.contract_external_id) AS contract_count,
        SUM(CAST(base.principal    AS DECIMAL(18,2))) AS principal_target,
...
async function getMonthlySummaryFromCache(
  ...
  // Phase 145+: ตรวจสอบ updated_at ด้วย — ถ้า cache ถูก populate วันก่อนหน้า (stale)
  //   ให้ fallback ไป live query เพราะ target ใช้ CURRENT_DATE ซึ่งเปลี่ยนทุกวัน
  //   (มีงวดใหม่ครบกำหนดทุกวัน ทำให้ตัวเลขเปลี่ยน)
  const checkSql = `
...
async function queryDueMonthTarget(
  ...
  const baseWhereForOuter = baseWhere.replace(/\bdtc\./g, "base.");
  // เมื่อมี buckets filter แล้ว ไม่ต้องมี NOT IN condition เพราะ bucket filter ครอบคลุมแล้ว
...
async function queryDueMonthDue(
  ...
  const baseWhereForOuter = baseWhere.replace(/\bdtc\./g, "base.");
  // เมื่อมี buckets filter แล้ว ไม่ต้องมี NOT IN condition เพราะ bucket filter ครอบคลุมแล้ว
  const notInFilter = (opts.buckets && opts.buckets.length > 0)
    ? ''
    : `AND COALESCE(dtc.contract_status, '') NOT IN ('ระงับสัญญา', 'สิ้นสุดสัญญา', 'หนี้เสีย', 'ยกเลิกสัญญา')`;
  const notInFilterOuter = (opts.buckets && opts.buckets.length > 0)
    ? ''
    : `AND COALESCE(base.contract_status, '') NOT IN ('ระงับสัญญา', 'สิ้นสุดสัญญา', 'หนี้เสีย', 'ยกเลิกสัญญา')`;
  const q = `
    SELECT
