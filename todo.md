...
### Phase 9Q — เพิ่ม penalty/unlockFee ในเป้าเก็บหนี้ + toggle + ซ่อนปิดค่างวดในยอดเก็บหนี้

- [x] ตรวจสอบ: penalty_due มี 813 งวด (max=1,400), unlock_fee_due มี 3,078 งวด (max=1,000) — ยืนยันต้องเพิ่มคอลัมน์
- [x] ตรวจสอบ: close_installment_amount = ยอดชำระปกติต่องวด (ซ้ำกับ principal+interest+fee) — ยืนยันควรซ่อน
- [x] Backend debtDb.ts: penalty + unlockFee อยู่ใน listDebtTarget response แล้ว (ตรวจสอบแล้ว)
- [x] Frontend เป้าเก็บหนี้: เพิ่มคอลัมน์ค่าปรับ + ค่าปลดล็อก ใน groupCols ของ target tab
- [x] Frontend เป้าเก็บหนี้: toggle checkbox "เฉพาะเงินต้น" ที่มุมขวาบนของตาราง — เมื่อติ๊ก ให้แสดงค่าปรับ/ค่าปลดล็อก = 0.00 (ไม่ใช่ซ่อนคอลัมน์)
