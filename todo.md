...
## UI Fixes - Mobile & Summary Page (01/05/2569)
- [ ] Mobile: เมนูซ้ายไม่แสดงเมนู "สัญญา" ในมือถือ
- [ ] Summary: สลับตำแหน่งฟิลเตอร์วันที่ชำระกับเดือน-ปีที่อนุมัติ
- [ ] Summary: ซ่อน Badge ค่าปรับ+ค่าปลดล็อกของเป้าเก็บหนี้เมื่อเปิดเฉพาะเงินต้น
- [ ] Summary: ปุ่ม Export เปลี่ยนเป็นสีเขียว
- [ ] Summary: Badge ที่ปิดตาแล้วให้สีจาง ไม่ต้องขีดฆ่า
- [ ] Summary: เลื่อนตารางซ้ายขวาไม่ได้บนจอเล็ก
- [ ] Summary: ไม่แสดงตารางเลยบนมือถือ
- [ ] Summary: แก้ logic คอลัมน์เป้าเก็บหนี้ (ยอดค้างชำระ/ยังไม่ชำระตั้งแต่งวดแรกถึงปัจจุบัน ไม่นับอนาคต)
- [ ] Summary: แก้ logic คอลัมน์ยอดเก็บหนี้ (ยอดค่างวดปกติที่ชำระแล้ว ไม่รวมยอดขายเครื่อง)
- [ ] Summary: แก้ logic คอลัมน์ยอดขายเครื่อง (ผลรวมยอดขายเครื่องของสัญญาทั้งหมดในเดือน-ปีที่อนุมัติ)
...
- [x] P133-1: เพิ่มปุ่มซ่อน/ขยาย Badge ทั้งเป้าเก็บหนี้และยอดเก็บหนี้ (กดทีเดียวซ่อนทั้งคู่)
- [x] P133-2: ลบข้อความ "ซ่อนแล้ว — คลิกไอคอนตาเพื่อแสดงอีกครั้ง" ออกจาก hidden row
- [x] P133-3: Commit + Push GitHub + Checkpoint

## Phase 134 — MonthlySummary แก้ไข (2026-05-01)
- [ ] P134-1: เพิ่มชื่อเมนู "สรุปรายเดือน" ที่ด้านบนซ้าย ใน row เดียวกับแถบ tab
- [ ] P134-2: เพิ่มไอคอน i popup อธิบายที่มาของตัวเลขในคอลัมน์ทั้งหมด
- [ ] P134-3: เอาปุ่มซ่อนตัวกรองออก
- [ ] P134-4: เพิ่มฟิลเตอร์ "ปีที่อนุมัติ" แบบ multi-select ต่อจาก "เดือน-ปีที่อนุมัติ"
- [ ] P134-5: เอาคอลัมน์ที่มีไอคอนปิดตาหลังสุดออกจากตารางทุกแถบ
- [ ] P134-6: แก้ bug ยอดที่ชำระแล้ว สถานะ เกิน1-7 ถึง เกิน31-60 = 0 ทั้งหมด
- [ ] P134-7: แก้ bug แถบ "ยอดที่ยังไม่ถึงกำหนด" ไม่มีข้อมูล
- [ ] P134-8: Commit + Push GitHub + Checkpoint

## Phase 131 — แก้ไขชื่อคอลัมน์ + สรุปรวม MonthlySummary

- [ ] DebtOverview: เปลี่ยน "เดือน" → "เดือน-ปีที่อนุมัติ" และ "จำนวนสัญญา" → "สัญญา" ในทุกจุด
- [ ] MonthlySummary: เปลี่ยน "เดือน" → "เดือน-ปีที่อนุมัติ" และ "จำนวนสัญญา" → "สัญญา" ในทุกจุด
...
- [x] P136-1: Badge ในแถบสรุปรวม ต้องไม่เลื่อนตามตาราง (ต้องอยู่นอก scroll container)
- [x] P136-2: BG ของคอลัมน์ "หัวข้อ" (sticky left-[130px]) ต้องไม่โปร่งใส เพื่อป้องกันข้อความซ้อน
- [x] P136-3: Badge "รวม" ในแถบสรุปรวม ให้รวมยอดขายเครื่อง (badDebt) ด้วย
- [x] P136-4: คอลัมน์ "รวม" ของแถว "ยอดเก็บหนี้" ในตาราง CombinedTable ให้รวมยอดขายเครื่องด้วย
- [x] P136-5: Commit + Push GitHub + Checkpoint

## Phase 137 — MonthlySummary CombinedTable % Tag + Export (2026-05-06)
- [x] P137-1: เพิ่ม % tag แถว "เป้าเก็บหนี้" คอลัมน์รวม = เป้าเก็บหนี้ / ยอดผ่อนรวม × 100 (สีน้ำเงิน)
- [x] P137-2: เพิ่ม % tag แถว "ยอดเก็บหนี้" คอลัมน์รวม 2 ตัวเลข: (1) ยอดเก็บหนี้/ยอดผ่อนรวม (สีม่วง) (2) ยอดเก็บหนี้/เป้าเก็บหนี้ (สีน้ำเงิน)
- [x] P137-3: เพิ่ม % tag แถว "หนี้ค้างชำระ" คอลัมน์รวม = หนี้ค้างชำระ / เป้าเก็บหนี้ × 100 (สีส้ม)
- [x] P137-4: เพิ่ม % tag แถว "ยังไม่ถึงกำหนด" คอลัมน์รวม = ยังไม่ถึงกำหนด / ยอดผ่อนรวม × 100 (สีน้ำเงิน)
...
## Bug Fix — ยอดเก็บหนี้ & เป้าเก็บหนี้ (2026-05-12)
- [x] ยอดเก็บหนี้: คอลัมน์งวดผ่อน — นับเฉพาะรายการที่ไม่ใช่ isBadDebtRow (ถ้าชำระแค่รายการหนี้เสียอย่างเดียว ต้องแสดง 0/N ไม่ใช่ 1/N)
- [x] ยอดเก็บหนี้: แสดงสัญญาที่ไม่มียอดชำระเลย (ปัจจุบันซ่อนอยู่ ค้นหาจากเลขที่สัญญาไม่เจอ)
- [x] เป้าเก็บหนี้: คอลัมน์งวดผ่อน — ถ้าไม่มียอดผ่อนเลยให้แสดงเป็น 1/N (ไม่ใช่ 0/N)
...
- [ ] Bug: ยอดเก็บหนี้ กรองสถานะ "ยกเลิกสัญญา" แสดงเฉพาะรายการที่มีการชำระ ไม่แสดงทั้งหมด
- [ ] Bug: เป้าเก็บหนี้ กรองสถานะ "ยกเลิกสัญญา" มาไม่ครบ 88 รายการ
- [ ] Bug: ป้ายกำกับ "ยกเลิกสัญญา" แสดงที่งวด N+1 ไม่ถูกต้อง (แสดงที่งวด 4 แทนงวด 3)
...
- [ ] Phase 139: สรุปรายเดือน — รวมสถานะ "ยกเลิกสัญญา" เข้าไปในตารางและ badge ด้วย (ปัจจุบันถูกกรองออก)
- [ ] Phase 140: สรุปรายเดือน — ยอดเก็บหนี้ไม่ตรงกับหน้ายอดเก็บหนี้และภาพรวมหนี้ (ต่างกัน ~2 ล้านบาท) ต้องแก้ queryPaid ใน monthlySummaryDb.ts
