...
### Phase 60 — Fix: isClosed maxNormalPeriod + carryCount for overpaid contracts
- [x] P60-1: วิเคราะห์ bug — สัญญาที่มี overpaid carry pool (TXRT-2 overpaid=7,802) มี TXRT-N suffix ไม่ตรงกับ period จริงๆ ทำให้ maxNormalPeriod ต่ำเกินไป งวด 6-7 แสดง "ปิดค่างวดแล้ว" ผิด
- [x] P60-2: แก้ Pass 2 ใน listDebtTargetStream — maxNormalPeriod = max(normalPeriods) + carryCount โดย carryCount = floor(totalOverpaid / baselineAmount) ต่อ contract
- [x] P60-3: TypeScript check ผ่าน 0 errors
- [x] P60-4: ทดสอบ CT0925-PKN001-15462-01: งวด 1-7 แสดงยอดปกติ ✅, งวด 8 ปิดค่างวดแล้ว ✅
- [x] P60-5: Commit + push GitHub + checkpoint

### Phase 61 — Fix: ยอดที่ชำระรวมใน collected stream ต้องรวม overpaid
